        ...
        <section style={{ margin: "0 48px 80px", borderRadius: 32, overflow: "hidden", background: `linear-gradient(135deg, ${C.tealDark} 0%, ${C.navyLight} 50%, ${C.navy} 100%)`, padding: "64px 56px", position: "relative" }}>
            ...
            <div style={{ position: "relative", display: "grid", gridTemplateColumns: "1fr auto", gap: 40, alignItems: "center" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 12, flexShrink: 0 }}>
                    <button onClick={() => setShowAppt(true)} style={{ background: C.white, border: "none", color: C.navy, padding: "14px 32px", borderRadius: 30, cursor: "pointer", fontSize: 14, fontWeight: 700, whiteSpace: "nowrap" }}>📅 Book Now</button>
                    <button onClick={() => setShowCard(true)} style={{ background: "transparent", border: `2px solid ${C.goldLight}`, color: C.goldLight, padding: "14px 32px", borderRadius: 30, cursor: "pointer", fontSize: 14, fontWeight: 700, whiteSpace: "nowrap" }}>💳 Get Card</button>
                </div>
            </div>
        </section>
