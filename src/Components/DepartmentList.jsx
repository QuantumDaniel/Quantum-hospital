                    ...
                    {departments.map((dept, i) => (
                        <div key={dept.id} style={{ background: C.white, borderRadius: 24, overflow: "hidden", border: `1px solid ${C.border}`, transition: "all 0.4s ease", cursor: "pointer", animation: `fadeUp 0.5s ease ${i * 0.06}s both` }}
                            ...
                            <div style={{ padding: 24 }}>
                                ...
                                <button onClick={() => openDept(dept)} style={{ width: "100%", background: `linear-gradient(135deg, ${dept.color}, ${dept.color}dd)`, border: "none", color: C.white, padding: "12px", borderRadius: 14, cursor: "pointer", fontSize: 14, fontWeight: 600, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>View Department →</button>
                            </div>
                        </div>
                    ))}
                </div>
